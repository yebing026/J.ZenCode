...
function initUI(_box){
	...
	if (KindEditor) {
		$('textarea.editor', $p).each(function(i){
			var $this = $(this);
			var options = {				
				fileManagerJson : fileManagerUrl,
				uploadJson:fileUpUrl,
			...
			var config = DWZ.jsonEval($this.attr("config"));
			if(config.type=='simple'){
			  		$.extend(options, optionsimple,config);		
					KindEditor.create(this,optionsimple);
				}else{
					$.extend(options, config);					
					KindEditor.create(this,options);
			}
		});
		$(':button[configimg]', $p).each(function(){
			var $this = $(this);
			var config = DWZ.jsonEval($this.attr("configimg"));
			...
			$(':button[configimgs]', $p).each(function(){
			var $this = $(this);
			var config = DWZ.jsonEval($this.attr("configimgs"));	
			var editor = KindEditor.editor($.extend({
						allowFileManager : true,
						fileManagerJson :fileManagerUrl,
						uploadJson : fileUpUrl
						...
						editor.plugin.multiImageDialog({
							clickFn : function(urlList) {
								...
								$.each(urlList, function(i, data) {
									div.append('<span class="pic">'+
	'<img  src="'+data.url+'" style="margin: 5px; opacity: 1; width:80px;">'+
"<br>"+
'<a onclick="delImg(\''+data.url+'\')" href="javascript:;" >删除</a></span>');
								});
		...
		$(':button[configfile]', $p).each(function(){
			var $this = $(this);
			var config = DWZ.jsonEval($this.attr("configfile"));	
			var uploadbutton = KindEditor.uploadbutton({
					button : this,
					fieldName : 'imgFile',
					url : config.uploadJson,
	...
	if ($.fn.uploadify) {
		$(":file[uploaderOption]", $p).each(function(){
			...
			var options = {
				...
				//onUploadComplete: uploadifyComplete,
