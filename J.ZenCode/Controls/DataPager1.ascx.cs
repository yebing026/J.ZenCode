...
public partial class Controls_DataPager1 : System.Web.UI.UserControl
{
    ...
    public void UpdatePaging(int pageIndex=1, int? recordCount = null)
    {
        if (recordCount == null)
        {
            try
            {
                recordCount = PageTool.GetRecordCount1(TableName, Where);
        ...
        if (recordCount != 0)
        {
            ...
            if (ctrl != null)
            {
                var dt = PageTool.GetPageData1(SelectFields, TableName, Where, OrderExpression, pageIndex, pageSize);
                if (ctrl is ListView)
                {
